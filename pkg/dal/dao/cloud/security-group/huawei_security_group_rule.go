...
func (dao *HuaWeiSGRuleDao) BatchCreateWithTx(kt *kit.Kit, tx *sqlx.Tx, rules []*cloud.HuaWeiSecurityGroupRuleTable) (
	...
	err = dao.Orm.ModifySQLOpts(orm.NewInjectTenantIDOpt(kt.TenantID)).Txn(tx).BulkInsert(kt.Ctx, sql, rules)
	if err != nil {
...
func (dao *HuaWeiSGRuleDao) batchCreateAudit(kt *kit.Kit, tx *sqlx.Tx,
	rules []*cloud.HuaWeiSecurityGroupRuleTable) error {
...
func (dao *HuaWeiSGRuleDao) UpdateWithTx(kt *kit.Kit, tx *sqlx.Tx, expr *filter.Expression, rule *cloud.
	...
	effected, err := dao.Orm.ModifySQLOpts(orm.NewInjectTenantIDOpt(kt.TenantID)).Txn(tx).Update(
...
func (dao *HuaWeiSGRuleDao) CountBySecurityGroupIDs(kt *kit.Kit, expr *filter.Expression) (map[string]int64, error) {
	...
	sql := fmt.Sprintf(`SELECT security_group_id, COUNT(*) AS rule_count FROM %s %s GROUP BY security_group_id;`,
		table.HuaWeiSecurityGroupRuleTable, whereExpr)
